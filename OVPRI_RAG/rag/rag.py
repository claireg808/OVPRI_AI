...
def re_rank(query, docs):
    ...
    average_score = sum(d[1] for d in scored_documents) / len(scored_documents)
    scored_documents.sort(key=lambda x: x[1], reverse=True)
    final_list = [d[0] for d in scored_documents if d[1] >= average_score]
    return final_list[:5]
...
def combine_docs(docs):
    combined_texts = []
    for d in docs:
        name = d.metadata.get('document_name', 'unknown').replace('-', ' - ')
...
def answer_query(query: str, history: list[str]) -> str:    
    ...
    filtered_docs = re_rank(query, docs)
    ...
    # create list of unique citations
    citations = set()
    citations_list = '\n\nRefer to these documents for more information:\n'
    for document in filtered_docs:
        name = document.metadata.get('document_name').replace('-', ' - ')
        citation = '* ' + name + '\n'
